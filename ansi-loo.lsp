...
; section added ***
#+lispworks ;; stuff for lispworks added by DC
(in-package :user)
#+lispworks
(eval-when (:compile-toplevel :load-toplevel :execute)
  (unless (find-package "ANSI-LOOP")
    (make-package "ANSI-LOOP" :use (list (find-package :cl)))))
#+(or lucid allegro aclpc clisp abcl sbcl)
(defpackage "ANSI-LOOP"
  (:use :cl)
  (:shadow #:loop #:loop-finish))
#+lispworks
(eval-when (:compile-toplevel :load-toplevel :execute)
   (shadow '(LOOP-FINISH LOOP) (find-package "ANSI-LOOP")))
#+lispworks
(eval-when (:compile-toplevel :load-toplevel :execute)
   (import 'SYSTEM::PROVIDE (find-package "ANSI-LOOP")))
...
#+lucid
(setf (macro-function 'ansi-loop::old-loop) (macro-function 'lisp::loop))
...
; section added ***
;; added by nmg, because lucid 4.0.x doesn't have REAL type
;; 2019-03 Don "LISP" => "COMMON-LISP" cause sbcl doesn't have package "LISP"
(unless (find-symbol "REAL" "COMMON-LISP") ;; doesn't have ansi REAL type
  (eval '(deftype real () '(or rational float))))

;;added by nmg -- is special-operator-p part of ANSI CL??
(eval-when (:compile-toplevel :load-toplevel :execute)
  (unless (fboundp 'special-operator-p)
    (when (fboundp 'special-form-p)
      ...
      (let ((answer
	      (cond ((null ncdrs)
		    ...
		    ((= ncdrs 0)
		     ...
		     #+ignore
		     `(setq ,tail-var (setf (cdr ,tail-var) ,tail-form))
