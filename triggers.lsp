...
#-old-rule-compiler
;; 2019-04-01 sbcl objects to declare special type
(defun find-triggers (vars wff type1 &optional stack
		      &aux result (currentvars vars))
  ; type1 is one of start, start*, start+
  (declare (special stack currentvars type1))
  ...
  (map-wff wff
	 ...
	 (case q
	   (a (case type1
		...
		(start* (let ((stack (cons `(and * (A ,v ,w)) stack))
			      (currentvars (append v currentvars))
			      (type1 'start)) ; could have been (and was) start+
			  (declare (special stack currentvars type1))
			  (map-wff-internal w)))
	 ...
	 (cond
	   ...
	   ((eq op 'or)
	    (loop for w in wffs do
		  (let ((stack (cons (cond ((eq type1 'start*) '*)
...
;; 2019-04-01 sbcl objects to declare special type
(defun push-in-start (vars wff type1 &aux result stack (currentvars vars))
  (declare (special stack currentvars type1 result))
