...
;; 2019-04 SBCL: (equal '`(,x) '`(,x))=>nil, equalp => t
;; and we have parameter lists with commas
(++ compare-relation (relationp 'parameter-list) 1 (relationp 'equalp))
    ...
    (labels ((keywd (name) (intern (symbol-name name) "KEYWORD"))
	  ...
	  (append (loop for u in union append
		  ...
		  (let ((arity (get-key :arity (car defrel-key-lists)))
		    ...
		    #+ignore ;; removed 2019-01 - called from defrel returning
                    ;; a new defrel form that might still infer an arity
...
(defun do-defrel (name arity implementation parameter-list equivs pdr
   ...
   (let* ((already (relationp name))
     ...
     (cond ((eql '#.(find-package "CL") (symbol-package name))
...
#+ignore ; better temporary def in ruledecl, final in tools
