  ...
  (labels
    ((arg-p (x which)
	 ...
	 (setf pkg (cond (tname (symbol-package tname))
			 ...
			 (t *package*)))
	 (when (eql pkg '#.(find-package "CL")) ;; LISP
 ...
 sharing a generator cache entry! |#
			...
			(if (and leave-constants-embedded
			 ...
			 (expand-arg arg
			   #'(lambda ()
			       (if (dbobject-p rel)
				   (if tailp ;; sbcl compiler points out this is nonsense
                                       ;; only in case of (apply r a b x)
                                       ;;(expand-wff-arg x r 3 t)
				       (loop for i from (1- pos) ;; best guess 2019 - formerly rel
					     below (arity* rel)
...
;; #+aclpc ;; next version adopted 2019-04
;; &rest fails for (?? . false) but &whole seems ok
;; this looks like true CL anyway, whereas the other is
;; debatable (atom as &rest) -- why not use &whole as the standard?
(defmacro ?? (&whole wff &rest rest &environment env)
  ;; somehow we need a rest arg in order to pass any args to the macro
