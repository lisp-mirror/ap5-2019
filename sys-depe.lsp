...
#+(or abcl lispworks (and lucid sparc))
...
#-lucid
(cl:defun compile (*fn-being-compiled* &rest args )
 ;;(let ((sys::*fasoutput-stream* nil)) ;; clisp bug?
   (apply #'cl::compile *fn-being-compiled* args))
 ;;)
...
#+TI
(cl:defun MyCompile (*fn-being-compiled* lambda-exp &aux
...
#-ti
(cl:defun MyCompile (*fn-being-compiled* lambda-exp)
  (with-optimizations ()
...
(cl::defun loop-for-s.t. (originalvars wff datatypes
		      &aux outputs pulser
		      (genfn (gentemp "GenFn "))
...
#+ignore 
(#+(or symbolics lucid) (shadow '(cl::loop) (find-package "AP5")) 
 ...
 (global:defmacro-displace loop (&environment e &rest body)
   (macroexpand-1 `(cl:loop ,.body) e))
 ; cl:loop is not displacing so we better macroexpand it
 ; expansion fn on symbolics seems not to use env ... 
 ; formerly (global:defmacro-displace loop (&rest body) `(cl:loop ,.body))
 ...
 (defmacro ap5::loop (&rest args &aux pos (result args) (start 0))
   (cl::loop while (setf pos (position-if #'(lambda (y)
					     (when (symbolp y) 
					       (string-equal "s.t."
						 (symbol-name y))))
					    result
...
(cl:defun compile-or-MExpand (name lambda-exp &optional dont-compile-p)
...
(defmacro defun (*fn-being-compiled* arglist &environment env &rest body
  ...
  (setf (get *fn-being-compiled* 'uses-generator) nil)
  (mexpand-all
	 `(lambda nil (cl::defun ,*fn-being-compiled* ,arglist ,.body))
	  env)
  ...
  `(progn
      ,.(loop for desc in (get *fn-being-compiled* 'uses-generator)
	    append (anon-rels-in-wff (third desc) env))
      ,@(save-generators-for-fn *fn-being-compiled*)
      (cl::defun ,*fn-being-compiled* ,arglist ,@body)))
  ...
  (let ((*fn-being-compiled* (gensym)))
	...
	`(progn ,@(save-generators-for-fn *fn-being-compiled*)
		(#+lucid clos:defmethod
		 #+allegro cl:defmethod
		 ,.body)
		)
      (setf (symbol-plist *fn-being-compiled*) nil))))


(cl::defun save-generators-for-fn (fname)
  (when (symbolp fname)
...
(cl::defun compile-code-for-desc
...
(progn
 ...
 (cl:defun initialize-resettable-bindings (name-of-list)
  (declare (cl:type list name-of-list))
 ...
 (cl:defun remember-variable-bindings (name value name-of-list)
  (declare (cl:type symbol name name-of-list))
...
#|
 ;; this seems to work in all of clisp/abcl/sbcl
 ;; though it seems annoying to make ap5 depend on not only bordeaux but 
 ;; now also asdf and alexandria
 (load "/home/don/twrepo/tw/code/3rdParty/asdf/asdf.lisp") ;; ASDF 3.3.2
 (in-package :asdf) ;; should really be in next file
 (load "/home/don/twrepo/tw/code/3rdParty//bordeaux-threads-master/bordeaux-threads.asd")
 (load "/home/don/twrepo/tw/code/3rdParty/alexandria-master/alexandria.asd")
 (require "bordeaux-threads")
 (in-package :ap5)
|#

;                                                ------ current-process(-name)
; transact.lsp - records current-process-name in history
; tools - similarly used in history function
; this file: do-with-whostate, do-withwriteaccess, do-writer, do-withwriteaccess

#+(or ti symbolics)
(defmacro current-process () 'global::current-process)
#+(or ti symbolics)
(cl:defun current-process-name () (global::send (current-process) :name))
#+allegro
...
(progn
 ...
 (cl:defun current-process-name () (mp:process-name (current-process))))
#+lucid
(progn
 (defmacro current-process () 'lcl:*current-process*)
 (cl:defun current-process-name () (lcl:process-name (current-process))))
#+lispworks ;; harlequin
(progn
 ; not a good thing to stick in a file, since it never returns
 ;(mp::initialize-multiprocessing)
 (defmacro current-process () 'mp:*current-process*)
 (cl:defun current-process-name ()
...
(progn
 ...
 (cl:defun current-process-name () (mt:thread-name (current-process))))

; 2019/03/15
#+abcl
(progn
 (defmacro current-process () '(threads:current-thread))
 (cl:defun current-process-name () (threads:thread-name (current-process))))
#+sbcl
(progn
 (defmacro current-process () 'sb-thread:*current-thread*)
 (cl:defun current-process-name () (sb-thread:thread-name (current-process))))

; for others we still need a process name to record in history
#-(or lucid ti allegro symbolics lispworks (and clisp mt) abcl sbcl)
(cl:defun current-process-name () "the only process")

#+bordeaux-threads ;; overwrite any of above
(progn
 (defmacro current-process () '(bordeaux-threads:current-thread))
 (cl:defun current-process-name () (bordeaux-threads:thread-name (current-process))))
...
of read/write locks.  And that's now only for systems that don't really have
threads (symbolics ti lucid?) plus some that maybe now do (allegro lispworks)
but in those cases the current code probably won't work - have to reimplement
withread/writeaccess
Previous uses:
 - ap5-package: import from lispworks -- removed 2019-03
 - transact: get-update-source nonatomic relations => withwriteaccess
 - utilities: there's still one, but under #+symbolics
...
;                                                       ----- with-whostate
;; uses:
;; generato: expand-msg (below)
;; transact: Trigger Automations,Check Consistency,Do Consistency,
;;           DerivedRels,Modify Database
;; triggers: Run Automations,ap-compile
...
(cl:defun do-with-whostate (whostate fn)
...
#+ignore ;; (and clisp mt) ; 2019-03 not worth having
(defmacro with-whostate (whostate &body body)
...
#+allegro
(cl:defun %pointer (x) (excl::pointer-to-address x))
#+lucid
(cl:defun %pointer (x) (system:%pointer x))
#+aclpc
(cl:defun %pointer (x) (acl::raw-address x))
#+clisp
(cl:defun %pointer (x) (SYSTEM::ADDRESS-OF x))
#+sbcl 
(cl:defun %pointer (x) (SB-KERNEL:GET-LISP-OBJ-ADDRESS x))
;; can't find one for abcl
#-(or symbolics ti lucid allegro aclpc clisp sbcl)
(cl::defun %pointer (x) (declare (ignore x)) "???")
;; the only purpose is to allow humans to compare printed
;; representations to determine eq-ness
;;  ??? is just as good as any of the others for machines
...
#-(or ti symbolics allegro lucid lispworks (and clisp mt) abcl sbcl)
(progn
...
(progn
 ...
 (cl:defun do-withreadaccess (fn)
  (declare #+symbolics (sys:downward-funarg fn)
           (cl:type fixnum |Readers |) ;; should be plenty!
 ...
 (cl:defun grab-readaccess-if-available ()
  (or |IHaveReadAccess |
      (without-interrupts
       (unless |Writer |
 ...
 #+(or ti lucid)
 (cl:defun do-withwriteaccess (fn)
 ...
 (cl:defun do-withwriteaccess (fn &aux (p (current-process)) gotit)
  #+symbolics (declare (sys:downward-funarg fn))
  (unwind-protect
      (progn
  ...
  (unwind-protect
      (progn (setf |Writer | (current-process))
	     (process-wait "waiting to write"
			   #'(lambda ()
			       (declare #-ti (cl:type (unsigned-byte 8) |Readers |))
...
(progn
 ...
 (cl:defun rwlock-read-lock (rwlock)
   (mt:with-mutex-lock ((rwlock-lock rwlock))
     (loop while (rwlock-active-writer rwlock) do ; write in progress
       ...
       (unwind-protect
           ...
           (progn (incf (rwlock-waiting-readers rwlock))
                  (mt:exemption-wait (rwlock-r-exemption rwlock)
                                     (rwlock-lock rwlock)))
          (decf (rwlock-waiting-readers rwlock))))
     (incf (rwlock-active-readers rwlock))))

 (cl:defun rwlock-read-unlock (rwlock)
   (mt:with-mutex-lock ((rwlock-lock rwlock))
 ...
 (cl:defun rwlock-write-lock (rwlock)
   (mt:with-mutex-lock ((rwlock-lock rwlock))
       ...
       (unwind-protect
             (progn (incf (rwlock-waiting-writers rwlock))
                    (mt:exemption-wait (rwlock-w-exemption rwlock)
                                    (rwlock-lock rwlock)))
           (decf (rwlock-waiting-writers rwlock))))
     (setf (rwlock-active-writer rwlock) t)))

 (cl:defun rwlock-write-unlock (rwlock)
   (mt:with-mutex-lock ((rwlock-lock rwlock))
...
#+sbcl
(progn ;; translate from clisp
 (defstruct rwlock
   (lock (sb-thread:make-mutex :name "rwlock-mutex"))
   (r-exemption (sb-thread:make-waitqueue :name "reader-exemption"))
   (w-exemption (sb-thread:make-waitqueue :name "writer-exemption"))
   (active-readers 0) ;; active readers
   (active-writer  nil) ;; is there write in progress
   (waiting-readers 0) ;; number of waiting readers
   (waiting-writers 0)) ;; number of waiting writers
 ...
 (cl:defun rwlock-read-lock (rwlock)
   (sb-thread:with-mutex ((rwlock-lock rwlock))
     (loop while (rwlock-active-writer rwlock) do ; write in progress
       ;; now supposed to be =
       ;; (mt:exemption-wait exemp lock :test (lambda nil (not (rwlock-active-writer rwlock)))
       (unwind-protect
           ...
           ;; The inner withreadaccess's are then noops.
           (progn (incf (rwlock-waiting-readers rwlock))
                  (sb-thread:condition-wait (rwlock-r-exemption rwlock)
                                            (rwlock-lock rwlock)))
         (decf (rwlock-waiting-readers rwlock))))
     (incf (rwlock-active-readers rwlock))))
 ...
 (cl:defun rwlock-read-unlock (rwlock)
   (sb-thread:with-mutex ((rwlock-lock rwlock))
     (when (and (zerop (decf (rwlock-active-readers rwlock))) ;; no more readers
                (plusp (rwlock-waiting-writers rwlock))) ;; have waiting writers
       (sb-thread:condition-notify (rwlock-w-exemption rwlock)))))
 ...
 (cl:defun rwlock-write-lock (rwlock)
   (sb-thread:with-mutex ((rwlock-lock rwlock))
     (loop while (or (rwlock-active-writer rwlock) (plusp (rwlock-active-readers rwlock))) do
       ...
       (unwind-protect
           (progn (incf (rwlock-waiting-writers rwlock))
                  (sb-thread:condition-wait (rwlock-w-exemption rwlock)
                                            (rwlock-lock rwlock)))
         (decf (rwlock-waiting-writers rwlock))))
     (setf (rwlock-active-writer rwlock) t)))
 ...
 (cl:defun rwlock-write-unlock (rwlock)
   (sb-thread:with-mutex ((rwlock-lock rwlock))
     ...
     (cond
       ...
       ;; give priority to writers
       ((plusp (rwlock-waiting-readers rwlock))
        (sb-thread:condition-broadcast (rwlock-r-exemption rwlock))))))
)
#+bordeaux-threads ;; again redefine any above
(progn ;; translate clisp to bordeaux
 (defstruct rwlock
   (lock (bordeaux-threads:make-lock "rwlock-mutex"))
   (r-exemption (bordeaux-threads:make-condition-variable))
   (w-exemption (bordeaux-threads:make-condition-variable))
   (active-readers 0) ;; active readers
   (active-writer  nil) ;; is there write in progress
   (waiting-readers 0) ;; number of waiting readers
   (waiting-writers 0)) ;; number of waiting writers
 ...
 (cl:defun rwlock-read-lock (rwlock)
   (bordeaux-threads:with-lock-held ((rwlock-lock rwlock))
     (loop while (rwlock-active-writer rwlock) do ; write in progress
       ;; now supposed to be =
       ;; (mt:exemption-wait exemp lock :test (lambda nil (not (rwlock-active-writer rwlock)))
       (unwind-protect
           ...
           ;; The inner withreadaccess's are then noops.
           (progn (incf (rwlock-waiting-readers rwlock))
                  (bordeaux-threads:condition-wait (rwlock-r-exemption rwlock)
                                                   (rwlock-lock rwlock)))
         (decf (rwlock-waiting-readers rwlock))))
     (incf (rwlock-active-readers rwlock))))
 ...
 (cl:defun rwlock-read-unlock (rwlock)
   (bordeaux-threads:with-lock-held ((rwlock-lock rwlock))
     (when (and (zerop (decf (rwlock-active-readers rwlock))) ;; no more readers
                (plusp (rwlock-waiting-writers rwlock))) ;; have waiting writers
       (bordeaux-threads:condition-notify (rwlock-w-exemption rwlock)))))
 ...
 (cl:defun rwlock-write-lock (rwlock)
   (bordeaux-threads:with-lock-held ((rwlock-lock rwlock))
     (loop while (or (rwlock-active-writer rwlock) (plusp (rwlock-active-readers rwlock))) do
       ...
       (unwind-protect
           (progn (incf (rwlock-waiting-writers rwlock))
                  (bordeaux-threads:condition-wait (rwlock-w-exemption rwlock)
                                                   (rwlock-lock rwlock)))
         (decf (rwlock-waiting-writers rwlock))))
     (setf (rwlock-active-writer rwlock) t)))
 ...
 (cl:defun rwlock-write-unlock (rwlock)
   (bordeaux-threads:with-lock-held ((rwlock-lock rwlock))
     ...
     (cond
       ...
       ;; give priority to writers
       ((plusp (rwlock-waiting-readers rwlock))
        ;; bordeaux has no condition-broadcast
        (loop for i below (rwlock-waiting-readers rwlock) do
          (bordeaux-threads:condition-notify (rwlock-r-exemption rwlock)))))))
)
#+(or (and clisp mt) abcl sbcl bordeaux-threads)
(progn
 ...
 (defmacro writing (&body body)
  `(do-writer #'(lambda () ,.body)))
 ;; 2019-03 in transact dodbupdates, do-simple-update inside with-writeaccess

 (cl:defun do-withreadaccess (fn &aux gotit)
...
#+(or (and clisp mt) abcl sbcl bordeaux-threads)
(progn ;; test thread implementation - (do-threads-test1) then see test-log1
  (defvar test-push-mutex
    #+sbcl(sb-thread:make-mutex :name "rwlock-mutex")
    #+(and clisp mt)(mt:make-mutex :name "rwlock-mutex")
    #+bordeaux-threads (bordeaux-threads:make-lock "rwlock-mutex")
    )
  (defmacro test-with-push-lock (&rest rest)
    `(#+(and clisp mt) mt:with-mutex-lock
      #+sbcl sb-thread:with-mutex
      #+abcl bordeaux-threads:with-lock-held
      (test-push-mutex) ,@rest))
  (defvar test-log1 nil)
  (setf test-log1 nil)
  (cl:defun threads-test1(i)
    (loop for i from 1 to i do
      (if (= 3 (mod i 6))
          (withwriteaccess
           (sleep .005)
           (test-with-push-lock
            (push (list (current-process-name) i :write 
                        :whohasw |WhoHasDataBaseWriteAccess |
                        :ihaver |IHaveReadAccess |
                        :activew (rwlock-active-writer rw-lock)
                        :activer (rwlock-active-readers rw-lock)
                        :waitw (rwlock-waiting-writers rw-lock)
                        :waitr (rwlock-waiting-readers rw-lock))
                  test-log1)))
        (withreadaccess
         (sleep .003)
         (test-with-push-lock
          (push (list (current-process-name) i :read 
                      :whohasw |WhoHasDataBaseWriteAccess |
                      :ihaver |IHaveReadAccess |
                      :activew (rwlock-active-writer rw-lock)
                      :activer (rwlock-active-readers rw-lock)
                ...
                test-log1))))
      (sleep .008)))
  (cl:defun do-threads-test1()
    (loop for i from 1 to 5 do
      (#+(and clisp mt)mt:make-thread
         #+sbcl sb-thread:make-thread
         #+bordeaux-threads bordeaux-threads:make-thread
         #-(or bordeaux-threads sbcl (and clisp mt)) error-no-make-thread
         (lambda ()(threads-test1 300)) :name (format nil "p~a" i)))))
...
(progn 
 (defmacro ignore-errors (&rest body)
  `(allegro::errorset 
     ...
     (values nil t)))
 (cl:defun unwind-error (&rest ignore) (declare (ignore ignore))
...
2019/03/30 the three occurrences in file triggers appear to be unused
It is not generally necessary that mexpand-all return the expansion
See uses above e.g. in def of defun.
|#
...
;  arg list
(cl:defun mexpand-all (x &optional env)
...
(progn
 (defstruct (loadmemodata :named (:conc-name LM-)
	    ...
	    data form)
 ;; lisp is not a standard nickname of pkg common-lisp, cl is
 (CL:defun print-ptr-lm (x stream depth)
 ...
 #+symbolics
 (progn  
  (cl:defun data-from (cell) ;; only used by interpreted calls on load-memo
    (lm-data cell))
  (compiler:add-optimizer data-from data-from-op)
  
  (cl:defun data-from-op
...
(progn
 ...
 (cl:defun print-ptr (x stream depth)
  (declare (ignore depth)) 
  (format stream "#<~Aap@~A>"
	  (if (done x) "!" "")
	  (%pointer x)))
...
(progn
 ...
 ;;; version that stores function in memo cell
 (cl:defun check-memo (cell)
  (declare (cl:type memodata cell))
  (cond ((done cell)
...
(eval-when (:compile-toplevel :load-toplevel :execute)
 #+(or ti symbolics)
 (cl:defun defstruct-access-function-name (struct slot)
 ...
 #+allegro
 (cl:defun defstruct-access-function-name
 ...
 #+lucid
 (cl:defun defstruct-access-function-name (structname slotname)
 ...
 (progn
  ...
  (if (string< "19" (lisp-implementation-version) "1999-01-03")
      ...
      (cl:defun new-ds-make-accessors (name type concname slotlist)
                  (cons `(setf (get ',name 'concname) ',concname)
                        (old-ds-make-accessors name type concname slotlist))) 
    
      ;; new version has 5 
      (cl:defun new-ds-make-accessors (name names type concname slotlist) 
                  (cons `(setf (get ',name 'concname) ',concname) 
...
(progn ; define dummy versions for those that are needed
 ...
 (cl:defun signal (x) nil) ; never used if no conditions
 (cl:defun invoke-restart (x) nil) ; never used if no conditions
