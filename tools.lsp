...
;; 2019-04 previous strategy of using cl (setf) documentation doesn't
;; work (as sbcl points out) - applies to only a few doc types
(defrelation doc :arity 3 :representation tree
  :types (symbol symbol string)
  :size ((input input output) 1)
  :count ((symbol symbol output) :countspec :optional :replacing t))

;; called by earlier code
(defun do-documentation (rel doc &optional (type 'relation)
			 &aux (relname (relationnamep rel)))
  #+ignore 
  ...
  (if doc (++ doc (or relname rel) type doc)
    (-- doc (or relname rel) type $)))
...
(defun describerel2 (rel stream extra-printer &aux temp)
  ...
  (loop for name s.t. (relationname rel name) do
    (loop for x s.t. (doc name 'relation x) do (terpri stream) (princ x stream)))
...
(defun DescribeRule (rule-or-name &optional (stream *standard-output*)
  ...
  (loop for name s.t. (rulename rule name) do
    (loop for doc s.t. (doc name 'rule doc) do
      (terpri stream) (princ doc stream)))
