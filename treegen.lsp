...
(defun treeadder
  ...
  (declare  (cl:type list tuple  compare-fns))
...
(defun treetester (tuple tree compare-rels)
  (declare (cl:type list tuple compare-rels) )
...
(defun treegen-common-from-data
  ...
  (let ((position-index
	...
	(declare (cl:type fixnum outputn))
	(setf (svref vec 0)
	      (make-treegen-cell
...
(defun treegen-internal (vec first nout)
  (declare (cl:type simple-vector vec) (cl:type fixnum nout))
  (let* ((maxslot (1- nout))
	 (slot (if first
		   ;; first time
		   0
	 ...
	 (cell (svref vec slot))
	 next-tree)
    (declare (cl:type fixnum maxslot slot))
